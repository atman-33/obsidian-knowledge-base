...
この vault では、フォルダごとに次のような役割を持たせます。

| フォルダ | 主な用途 | 置くものの例 |
|---|---|---|
| `0. Common/` | vault 全体で共通に使う運用ファイル | `index.md`、`log.md`、日次メモ、ダッシュボード |
| `1. Projects/` | 期限・成果物のある進行中の取り組み | 案件ノート、計画、タスク、打ち合わせメモ |
| `2. Areas/` | 終了日を持たない継続的な責任領域 | Career、健康、家庭、継続業務の管理ノート |
| `3. Resources/` | 参照用の知識・資料 | 技術メモ、調査ノート、チートシート、読書メモ |
| `4. Archive/` | 完了・停止したものの保管 | 完了済みプロジェクト、古い資料、過去ログ |

使い分けの目安は次のとおりです。

- 終了日があり、完了を目指すものは `1. Projects/`
- 継続して維持・改善する責任領域は `2. Areas/`
- 今後参照するために残す知識や資料は `3. Resources/`
- すでにアクティブではないものは `4. Archive/`
- vault 全体の共通運用に関わるものは `0. Common/`

`0. Common/` は標準的な PARA の 4 区分に対して、この vault で追加している共通運用レイヤーです。
そのため、個別テーマのノートを集約する場所ではなく、vault 全体で共有するファイルに限定して使うのが推奨です。
