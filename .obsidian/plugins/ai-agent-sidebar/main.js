...
__export(shellEnv_exports, {
  resolveShellEnv: () => resolveShellEnv
});
function resolveShellEnv() {
  if (resolvedEnvPromise) return resolvedEnvPromise;
  resolvedEnvPromise = new Promise((resolve2) => {
    var _a3, _b;
    const shell = (_a3 = process.env.SHELL) != null ? _a3 : "/bin/bash";
    let proc;
    try {
      proc = (0, import_child_process.spawn)(shell, ["-l", "-c", "env"], {
        shell: false,
    ...
    let out = "";
    (_b = proc.stdout) == null ? void 0 : _b.on("data", (d2) => out += d2.toString());
    proc.on("close", () => {
    ...
    proc.on("error", () => resolve2({ ...process.env }));
  });
  return resolvedEnvPromise;
}
var import_child_process, resolvedEnvPromise;
          ...
          if (fetch3.isRedirect(res.statusCode)) {
            ...
            switch (request.redirect) {
              ...
              case "follow":
                ...
                resolve2(fetch3(new Request5(locationURL, requestOpts)));
                finalize();
                return;
            }
          }
...
var import_fs, import_path2, import_node_domexception, __classPrivateFieldSet4, __classPrivateFieldGet5, _FileFromPath_path, _FileFromPath_start, MESSAGE, FileFromPath;
...
var AgentDetector = class {
  ...
  async detect(adapters) {
    if (this.cache) return this.cache;
    return this.rescan(adapters);
  }
  async rescan(adapters) {
    if (!adapters) {
      if (!this.cache) return [];
      adapters = this.cache.map((r2) => ({
        id: r2.id,
        name: r2.name,
        command: r2.command,
  ...
  async detectStream(adapters, onResult) {
    const shellEnv = await resolveShellEnv();
  ...
  async detectOne(adapter, shellEnv) {
    var _a3, _b, _c2, _d;
    const provider = PROVIDERS.find((p2) => p2.agentId === adapter.id);
    const primaryVar = (_b = (_a3 = provider == null ? void 0 : provider.apiKeyEnvVar) != null ? _a3 : adapter.apiKeyVar) != null ? _b : "";
    const candidateVars = [
      ...primaryVar ? [primaryVar] : [],
      ...(_c2 = provider == null ? void 0 : provider.fallbackApiKeyEnvVars) != null ? _c2 : []
    ];
    const foundVar = candidateVars.find((v2) => Boolean(shellEnv[v2]));
    const hasApiKey = Boolean(foundVar);
    const apiKeyVar = foundVar != null ? foundVar : primaryVar;
    if (!(provider == null ? void 0 : provider.cliSupported)) {
      return {
        id: adapter.id,
        name: adapter.name,
        command: (_d = adapter.command) != null ? _d : "",
        path: "",
        isInstalled: false,
        // When no API key env var is required, treat as always available
        hasApiKey: (provider == null ? void 0 : provider.apiKeyOptional) ? true : hasApiKey,
        apiKeyVar
      };
    }
    let whichCmd;
    ...
    try {
      const { stdout } = await execAsync(whichCmd, { timeout: 1e4 });
  ...
  unavailable(adapter, hasApiKey = false, apiKeyVar = "") {
    return {
      id: adapter.id,
      name: adapter.name,
      command: adapter.command,
...
init_shellEnv();
var MAX_CONTEXT_BYTES = 8 * 1024;
var _c = String.fromCharCode;
var ESC = _c(27);
var BEL = _c(7);
var STRIP_CSI_RE = new RegExp(ESC + "\\[[0-9;?]*[A-Za-z@`]", "g");
var STRIP_OSC_RE = new RegExp(ESC + "\\][\\s\\S]*?(?:" + BEL + "|" + ESC + "\\\\)", "g");
var STRIP_DCS_RE = new RegExp(ESC + "[PX^_][\\s\\S]*?" + ESC + "\\\\", "g");
var STRIP_TWO_RE = new RegExp(ESC + "[\\s\\S]", "g");
var STRIP_ESC_RE = new RegExp(ESC, "g");
var STRIP_CTRL_RE = new RegExp(
  "[" + _c(0) + "-" + _c(8) + _c(11) + _c(12) + _c(14) + "-" + _c(31) + _c(127) + "-" + _c(159) + "]",
  "g"
);
var FILE_OP_OPEN = ":::file-op";
var FILE_OP_CLOSE = ":::";
var AGENT_ADAPTERS = [
  {
    id: "claude",
...
var AgentRunner = class extends import_events.EventEmitter {
  constructor(adapter, binaryPath, extraArgs, fileOpsHandler) {
    super();
  ...
  buildSystemPrompt(vaultPath, activeFileContent) {
...
--- END VAULT CONTEXT ---
` : "";
    return `You are an AI assistant integrated into Obsidian via the AI Agent Sidebar plugin.
...
`;
  ...
  async run(messages, context) {
    const { vaultPath, activeFileContent } = JSON.parse(context);
    const lastMsg = messages[messages.length - 1];
    const history = messages.slice(0, -1);
    await this.sendMessage(lastMsg.content, history, vaultPath, activeFileContent);
  }
  async sendMessage(userMessage, history, vaultPath, activeFileContent) {
    if (this.disposed) {
  ...
  buildPromptForOneShot(systemPrompt, history, userMessage) {
...
Assistant:`);
  ...
  async spawnAndStream(prompt, args) {
    const shellEnv = await resolveShellEnv();
    return new Promise((resolve2) => {
      ...
      const promptArgs = this.adapter.promptFlag ? [this.adapter.promptFlag, prompt] : [prompt];
      const spawnArgs = useStdin ? args : [...args, ...promptArgs];
      const proc = (0, import_child_process3.spawn)(this.binaryPath, spawnArgs, {
        shell: false,
...
[timed out after ${TIMEOUT_MS / 1e3}s \u2014 process killed]
`);
        proc.kill("SIGTERM");
      }, TIMEOUT_MS);
      let stdoutBuffer = "";
      ...
      let textAccumulator = "";
      let pendingChunks = 0;
      let processCloseCode = void 0;
      const stripAnsi = (s2) => s2.replace(/\r\n/g, "\n").replace(/\r/g, "\n").replace(STRIP_CSI_RE, "").replace(STRIP_OSC_RE, "").replace(STRIP_DCS_RE, "").replace(STRIP_TWO_RE, "").replace(STRIP_ESC_RE, "").replace(STRIP_CTRL_RE, "");
      const flushText = () => {
        if (textAccumulator) {
      ...
      const processChunk = async (chunk) => {
        ...
        while (true) {
          if (!inFileOpBlock) {
            ...
            const actualClose = closeIdx !== -1 ? closeIdx : closeIdxEnd !== -1 && closeIdxEnd + FILE_OP_CLOSE.length >= stdoutBuffer.length - 1 ? closeIdxEnd : -1;
            if (actualClose === -1) {
      ...
      proc.stdout.on("data", (data) => {
        ...
        pendingChunks++;
        processChunk(stripped).catch((err) => console.error("[AgentRunner] processChunk error:", err)).finally(() => {
          pendingChunks--;
          if (pendingChunks === 0 && processCloseCode !== void 0) {
      ...
      proc.on("close", (code) => {
        ...
        this.emit("raw", "stderr", `
[process exited with code ${code != null ? code : "null"}]
`);
        if (pendingChunks === 0) {
          finalise(code);
        } else {
...
// src/icons.ts
function appendAgentIcon(container, agentId) {
...
var AGENT_ICONS = {
  ...
  // OpenAI Codex — simpleicons.org/icons/openai.svg
  codex: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
  ...
  // Google Gemini — simpleicons.org/icons/googlegemini.svg
  gemini: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
  ...
  // OpenAI Compatible — generic server/API icon (24x24)
  "openai-compat": `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
  ...
  // GitHub Copilot — simpleicons.org/icons/githubcopilot.svg
  copilot: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
...
// src/AgentChatTab.ts
var MAX_CONTEXT_BYTES2 = 8 * 1024;
var AgentChatTab = class {
  constructor(containerEl, runner, detection, app, plugin) {
    this.history = [];
  ...
  get modeLabel() {
    var _a3, _b;
    const mode = (_b = (_a3 = this.plugin.settings.agents[this.detection.id]) == null ? void 0 : _a3.accessMode) != null ? _b : "cli";
  ...
  renderFileOpCard(parentEl, record) {
    var _a3, _b, _c2;
    ...
    icon.textContent = (_a3 = opIcons[op.op]) != null ? _a3 : "\u{1F4C4}";
    ...
    const pathText = (_c2 = (_b = op.path) != null ? _b : op.oldPath) != null ? _c2 : "";
    if (pathText) {
  ...
  bindRunnerEvents() {
    this.runner.on("raw", (stream, text) => {
    ...
    this.runner.on("stderr", (text) => {
      if (this.currentAssistantMsgEl && this.statusEl) {
        this.statusEl.textContent = text;
        this.scrollToBottom();
      }
    });
    this.runner.on("token", (text) => {
      var _a3;
      if (this.statusEl) {
      ...
      const contentEl = (_a3 = this.currentAssistantMsgEl) == null ? void 0 : _a3.querySelector(".ai-sidebar-message-content");
      if (contentEl) contentEl.textContent = this.currentAssistantContent;
      this.scrollToBottom();
    });
    this.runner.on("fileOpStart", (op) => {
      var _a3, _b;
      if (this.statusEl) {
        this.statusEl.remove();
        this.statusEl = null;
      ...
      pendingCard.textContent = `${fileOp.op.toUpperCase()} ${(_b = (_a3 = fileOp.path) != null ? _a3 : fileOp.oldPath) != null ? _b : ""}\u2026`;
    });
    this.runner.on("fileOpResult", (op, result) => {
  ...
  renderError(message) {
    ...
    retryBtn.addEventListener("click", () => {
      errorEl.remove();
      const lastUserMsg = [...this.history].reverse().find((m2) => m2.role === "user");
      if (lastUserMsg) {
        this.history.pop();
  ...
  async sendMessageContent(text) {
    ...
    if (activeFile) {
      try {
        ...
        if (activeFileContent.length > MAX_CONTEXT_BYTES2) {
          activeFileContent = activeFileContent.slice(0, MAX_CONTEXT_BYTES2);
        }
      } catch (e2) {
      }
    }
    const vaultPath = (_a3 = this.app.vault.adapter.basePath) != null ? _a3 : "";
...
var FileOperationsHandler = class {
  constructor(app) {
    var _a3;
    this.app = app;
    this.vaultRoot = (_a3 = this.app.vault.adapter.basePath) != null ? _a3 : "";
  }
  async execute(op) {
    ...
    try {
      switch (op.op) {
        case "read":
          return await this.read((_a3 = op.path) != null ? _a3 : "");
        case "write":
          return await this.write((_b = op.path) != null ? _b : "", (_c2 = op.content) != null ? _c2 : "");
        case "delete":
          return await this.delete((_d = op.path) != null ? _d : "");
        case "rename":
          return await this.rename((_e2 = op.oldPath) != null ? _e2 : "", (_f = op.newPath) != null ? _f : "");
        case "list":
          return this.list((_g = op.path) != null ? _g : "");
        default:
  ...
  validatePath(relativePath) {
    ...
    const normalized = (0, import_obsidian.normalizePath)(relativePath);
    const resolved = this.vaultRoot ? (0, import_path.resolve)(this.vaultRoot, normalized) : resolved_fallback(normalized);
  ...
  list(path) {
    ...
    const target = safePath ? this.app.vault.getAbstractFileByPath(safePath) : this.app.vault.getRoot();
    if (!target || !(target instanceof import_obsidian.TFolder)) {
      return { ok: false, error: `Folder not found: ${safePath || "/"}` };
    }
    const entries = target.children.map((child) => ({
      name: child.name,
      path: child.path,
      type: child instanceof import_obsidian.TFolder ? "folder" : "file"
    }));
...
function resolved_fallback(path) {
  return path;
}
...
var deprecateConstructorEntries = (0, import_util2.deprecate)(() => {
...
async function toFile(value, name, options) {
  ...
  if (!(options == null ? void 0 : options.type)) {
    const type = (_c2 = bits[0]) == null ? void 0 : _c2.type;
...
var APIClient = class {
  constructor({
    ...
    this.fetch = overriddenFetch != null ? overriddenFetch : fetch;
  }
  authHeaders(opts) {
    return {};
  }
  ...
  buildRequest(options, { retryCount = 0 } = {}) {
    ...
    const httpAgent = (_c2 = (_b = options.httpAgent) != null ? _b : this.httpAgent) != null ? _c2 : getDefaultAgent(url);
...
var readEnv = (env) => {
  ...
  if (typeof process !== "undefined") {
    return (_c2 = (_b = (_a3 = process.env) == null ? void 0 : _a3[env]) == null ? void 0 : _b.trim()) != null ? _c2 : void 0;
  }
...
var BetaMessageStream = class _BetaMessageStream {
  ...
  _connected(response) {
    ...
    __classPrivateFieldSet7(this, _BetaMessageStream_request_id, response == null ? void 0 : response.headers.get("request-id"), "f");
  ...
  _emit(event, ...args) {
    ...
    if (event === "abort") {
      const error = args[0];
      if (!__classPrivateFieldGet8(this, _BetaMessageStream_catchingPromiseCreated, "f") && !(listeners == null ? void 0 : listeners.length)) {
      ...
      case "content_block_delta": {
        ...
        switch (event.delta.type) {
          ...
          case "citations_delta": {
            if (content.type === "text") {
              this._emit("citation", event.delta.citation, (_a3 = content.citations) != null ? _a3 : []);
            }
            break;
          }
    ...
    switch (event.type) {
      ...
      case "content_block_delta": {
        ...
        switch (event.delta.type) {
          ...
          case "citations_delta": {
            if ((snapshotContent == null ? void 0 : snapshotContent.type) === "text") {
              (_a3 = snapshotContent.citations) != null ? _a3 : snapshotContent.citations = [];
...
Please migrate to a newer model. Visit https://docs.anthropic.com/en/docs/resources/model-deprecations for more information.`);
    ...
    return this._client.post("/v1/messages?beta=true", {
      body,
      timeout: (_a3 = this._client._options.timeout) != null ? _a3 : body.stream ? 6e5 : this._client._calculateNonstreamingTimeout(body.max_tokens),
      ...options,
      headers: {
...
var MessageStream = class _MessageStream {
  ...
  _connected(response) {
    ...
    __classPrivateFieldSet8(this, _MessageStream_request_id, response == null ? void 0 : response.headers.get("request-id"), "f");
  ...
  _emit(event, ...args) {
    ...
    if (event === "abort") {
      const error = args[0];
      if (!__classPrivateFieldGet9(this, _MessageStream_catchingPromiseCreated, "f") && !(listeners == null ? void 0 : listeners.length)) {
        Promise.reject(error);
      }
...
var AnthropicProvider = class {
  ...
  async *stream(messages, context, model) {
    const { vaultPath, activeFileContent } = JSON.parse(context);
    const systemPrompt = buildSystemPrompt(vaultPath, activeFileContent);
    const anthropicMessages = messages.map((m2) => ({
  ...
  async listModels() {
    ...
    return resp.data.map((m2) => m2.id);
  }
};
function buildSystemPrompt(vaultPath, activeFileContent) {
  const MAX_CONTEXT_BYTES3 = 8 * 1024;
  const truncated = activeFileContent ? activeFileContent.slice(0, MAX_CONTEXT_BYTES3) : null;
  const contextSection = truncated ? `
--- BEGIN VAULT CONTEXT (read-only reference) ---
${truncated}
--- END VAULT CONTEXT ---
      ...
      try {
        for await (const sse of _iterSSEMessages2(response, controller)) {
          ...
          if (sse.event === null) {
            ...
            if (data && data.error) {
              throw new APIError2(void 0, data.error, void 0, void 0);
            }
            yield data;
          } else {
            let data;
...
var APIPromise2 = class _APIPromise extends Promise {
  ...
  _thenUnwrap(transform) {
    return new _APIPromise(this.responsePromise, async (props) => _addRequestID2(transform(await this.parseResponse(props), props), props.response));
  }
  /**
   * Gets the raw `Response` instance instead of parsing the response
   * data.
...
var APIClient2 = class {
  ...
  buildRequest(options, { retryCount = 0 } = {}) {
    ...
    options.timeout = (_a3 = options.timeout) != null ? _a3 : this.timeout;
    const httpAgent = (_c2 = (_b = options.httpAgent) != null ? _b : this.httpAgent) != null ? _c2 : getDefaultAgent2(url);
    const minAgentTimeout = options.timeout + 1e3;
    if (typeof ((_d = httpAgent == null ? void 0 : httpAgent.options) == null ? void 0 : _d.timeout) === "number" && minAgentTimeout > ((_e2 = httpAgent.options.timeout) != null ? _e2 : 0)) {
      httpAgent.options.timeout = minAgentTimeout;
    }
...
var EventStream = class {
  constructor() {
    ...
    __classPrivateFieldSet11(this, _EventStream_connectedPromise, new Promise((resolve2, reject) => {
      __classPrivateFieldSet11(this, _EventStream_resolveConnectedPromise, resolve2, "f");
      __classPrivateFieldSet11(this, _EventStream_rejectConnectedPromise, reject, "f");
    }), "f");
    __classPrivateFieldSet11(this, _EventStream_endPromise, new Promise((resolve2, reject) => {
      __classPrivateFieldSet11(this, _EventStream_resolveEndPromise, resolve2, "f");
  ...
  _emit(event, ...args) {
    ...
    if (event === "abort") {
      const error = args[0];
      if (!__classPrivateFieldGet12(this, _EventStream_catchingPromiseCreated, "f") && !(listeners == null ? void 0 : listeners.length)) {
        Promise.reject(error);
      }
...
_AbstractChatCompletionRunner_instances = /* @__PURE__ */ new WeakSet(), _AbstractChatCompletionRunner_getFinalContent = function _AbstractChatCompletionRunner_getFinalContent2() {
  var _a3;
  return (_a3 = __classPrivateFieldGet13(this, _AbstractChatCompletionRunner_instances, "m", _AbstractChatCompletionRunner_getFinalMessage).call(this).content) != null ? _a3 : null;
...
var ChatCompletionStream = class _ChatCompletionStream extends AbstractChatCompletionRunner {
  ...
  [(_ChatCompletionStream_params = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_choiceEventStates = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_currentChatCompletionSnapshot = /* @__PURE__ */ new WeakMap(), _ChatCompletionStream_instances = /* @__PURE__ */ new WeakSet(), _ChatCompletionStream_beginRequest = function _ChatCompletionStream_beginRequest2() {
    ...
    var _a3, _b, _c2, _d, _e2, _f, _g, _h, _i, _j, _k, _l, _m, _n, _o, _p, _q, _r2, _s, _t2;
    if (this.ended)
      return;
    ...
    for (const choice of chunk.choices) {
      ...
      if (choice.delta.refusal != null && ((_c2 = choiceSnapshot.message) == null ? void 0 : _c2.role) === "assistant" && ((_d = choiceSnapshot.message) == null ? void 0 : _d.refusal)) {
    ...
    if (toolCallSnapshot.type === "function") {
      const inputTool = (_c2 = (_b = __classPrivateFieldGet14(this, _ChatCompletionStream_params, "f")) == null ? void 0 : _b.tools) == null ? void 0 : _c2.find((tool) => tool.type === "function" && tool.function.name === toolCallSnapshot.function.name);
    ...
    var _a3;
    const responseFormat = (_a3 = __classPrivateFieldGet14(this, _ChatCompletionStream_params, "f")) == null ? void 0 : _a3.response_format;
    if (isAutoParsableResponseFormat(responseFormat)) {
      return responseFormat;
    }
    return null;
  }, _ChatCompletionStream_accumulateChatCompletion = function _ChatCompletionStream_accumulateChatCompletion2(chunk) {
    var _a4, _b2, _c3, _d2, _e2, _f;
    var _a3, _b, _c2, _d;
      ...
      if (function_call) {
        if (!choice.message.function_call) {
          ...
          if (function_call.arguments) {
            (_c3 = (_c2 = choice.message.function_call).arguments) != null ? _c3 : _c2.arguments = "";
...
${str(snapshot)}`);
      ...
      return {
        ...choiceRest,
        message: { ...messageRest, content, role, refusal: (_c2 = message.refusal) != null ? _c2 : null },
        finish_reason,
        index,
        logprobs
      };
...
_AssistantStream_addEvent = function _AssistantStream_addEvent2(event) {
  ...
  switch (event.event) {
    ...
    case "thread.message.completed":
    case "thread.message.incomplete":
      if (__classPrivateFieldGet15(this, _AssistantStream_currentContentIndex, "f") !== void 0) {
      ...
      __classPrivateFieldSet13(this, _AssistantStream_messageSnapshot, void 0, "f");
  }
  ...
  switch (event.event) {
    ...
    case "thread.run.step.cancelled":
    case "thread.run.step.expired":
      __classPrivateFieldSet13(this, _AssistantStream_currentRunStepSnapshot, void 0, "f");
      const details = event.data.step_details;
      if (details.type == "tool_calls") {
        if (__classPrivateFieldGet15(this, _AssistantStream_currentToolCall, "f")) {
          this._emit("toolCallDone", __classPrivateFieldGet15(this, _AssistantStream_currentToolCall, "f"));
          __classPrivateFieldSet13(this, _AssistantStream_currentToolCall, void 0, "f");
        }
      }
...
var OpenAIProvider = class {
  ...
  async *stream(messages, context, model) {
    var _a3, _b;
    const { vaultPath, activeFileContent } = JSON.parse(context);
    const systemPrompt = buildSystemPrompt2(vaultPath, activeFileContent);
  ...
  async listModels() {
    const resp = await this.client.models.list();
    return resp.data.map((m2) => m2.id).filter(filterOpenAIModelId).sort();
  }
};
function filterOpenAIModelId(id) {
  return id.startsWith("gpt-") || /^o\d/.test(id);
}
function buildSystemPrompt2(vaultPath, activeFileContent) {
  const MAX_CONTEXT_BYTES3 = 8 * 1024;
  const truncated = activeFileContent ? activeFileContent.slice(0, MAX_CONTEXT_BYTES3) : null;
  const contextSection = truncated ? `
--- BEGIN VAULT CONTEXT (read-only reference) ---
...
var GeminiProvider = class {
  ...
  async *stream(messages, context, model) {
    var _a3, _b, _c2, _d, _e2, _f;
    const { vaultPath, activeFileContent } = JSON.parse(context);
    const systemInstruction = buildSystemPrompt3(vaultPath, activeFileContent);
    const contents = mergeGeminiMessages(messages);
    if (contents.length === 0 || contents[contents.length - 1].role !== "user") {
      return;
    }
    const baseUrl = ((_a3 = this.baseURL) != null ? _a3 : "https://generativelanguage.googleapis.com").replace(/\/$/, "");
    const url = `${baseUrl}/v1beta/models/${model}:streamGenerateContent?alt=sse`;
    const r2 = await (0, import_obsidian2.requestUrl)({
      url,
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    if (r2.status !== 200) {
      throw new Error(`Gemini API error: ${r2.status}`);
    }
    const text = r2.text;
    for (const line of text.split("\n")) {
      ...
      try {
        ...
        const candidateText = (_f = (_e2 = (_d = (_c2 = (_b = data.candidates) == null ? void 0 : _b[0]) == null ? void 0 : _c2.content) == null ? void 0 : _d.parts) == null ? void 0 : _e2[0]) == null ? void 0 : _f.text;
  ...
  async listModels() {
    ...
    const data = r2.json;
    return data.models.filter((m2) => {
      var _a3;
      return (_a3 = m2.supportedGenerationMethods) == null ? void 0 : _a3.includes("generateContent");
    }).map((m2) => m2.name.replace("models/", ""));
  }
};
function mergeGeminiMessages(messages) {
  const contents = [];
...
function buildSystemPrompt3(vaultPath, activeFileContent) {
  const MAX_CONTEXT_BYTES3 = 8 * 1024;
  const truncated = activeFileContent ? activeFileContent.slice(0, MAX_CONTEXT_BYTES3) : null;
  const contextSection = truncated ? `
--- BEGIN VAULT CONTEXT (read-only reference) ---
...
var OpenAICompatProvider = class {
  constructor(apiKey, baseURL) {
    this.client = new openai_default({
      // OpenAI SDK requires a non-empty apiKey; use a placeholder when none is needed (e.g. Ollama)
  ...
  async *stream(messages, context, model) {
    var _a3, _b;
    const { vaultPath, activeFileContent } = JSON.parse(context);
    const systemPrompt = buildSystemPrompt4(vaultPath, activeFileContent);
    const openAIMessages = [
      { role: "system", content: systemPrompt },
      ...messages.map((m2) => ({
        role: m2.role === "assistant" ? "assistant" : "user",
  ...
  async listModels() {
    const resp = await this.client.models.list();
    return resp.data.map((m2) => m2.id).sort();
  }
};
function buildSystemPrompt4(vaultPath, activeFileContent) {
  const MAX_CONTEXT_BYTES3 = 8 * 1024;
  const truncated = activeFileContent ? activeFileContent.slice(0, MAX_CONTEXT_BYTES3) : null;
  const contextSection = truncated ? `
--- BEGIN VAULT CONTEXT (read-only reference) ---
...
// src/AgentApiRunner.ts
var FILE_OP_OPEN2 = ":::file-op";
var FILE_OP_CLOSE2 = ":::";
var INACTIVITY_TIMEOUT_MS = 3e4;
function sanitiseError(message, apiKey) {
  return message.replace(new RegExp(apiKey.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "g"), "[REDACTED]");
}
var AgentApiRunner = class extends import_events2.EventEmitter {
  constructor(agentId, apiKey, model, fileOpsHandler, debugMode = false, baseURL, provider) {
    super();
    this.disposed = false;
    ...
    this.provider = provider != null ? provider : this.createProvider(agentId, apiKey, baseURL);
  }
  debug(text) {
    if (this.debugMode) this.emit("raw", "stdout", text);
  }
  createProvider(agentId, apiKey, baseURL) {
    if (agentId === "claude") return new AnthropicProvider(apiKey, baseURL);
    if (agentId === "codex") return new OpenAIProvider(apiKey, baseURL);
    if (agentId === "gemini") return new GeminiProvider(apiKey, baseURL);
    if (agentId === "openai-compat") return new OpenAICompatProvider(apiKey, baseURL != null ? baseURL : "http://localhost:11434/v1");
  ...
  async run(messages, context) {
    ...
    const processChunk = async (chunk) => {
      ...
      while (true) {
        if (!inFileOpBlock) {
          const openIdx = stdoutBuffer.indexOf(FILE_OP_OPEN2);
          if (openIdx === -1) {
            const safeLen = Math.max(0, stdoutBuffer.length - (FILE_OP_OPEN2.length - 1));
            textAccumulator += stdoutBuffer.slice(0, safeLen);
            stdoutBuffer = stdoutBuffer.slice(safeLen);
            flushText();
            break;
          ...
          const closeIdx = stdoutBuffer.indexOf(FILE_OP_CLOSE2 + "\n");
          const closeIdxEnd = stdoutBuffer.indexOf(FILE_OP_CLOSE2);
          const actualClose = closeIdx !== -1 ? closeIdx : closeIdxEnd !== -1 && closeIdxEnd + FILE_OP_CLOSE2.length >= stdoutBuffer.length - 1 ? closeIdxEnd : -1;
          if (actualClose === -1) {
            const safeLen = Math.max(0, stdoutBuffer.length - (FILE_OP_CLOSE2.length - 1));
            fileOpBuffer += stdoutBuffer.slice(0, safeLen);
            stdoutBuffer = stdoutBuffer.slice(safeLen);
            break;
          }
          fileOpBuffer += stdoutBuffer.slice(0, actualClose);
          stdoutBuffer = stdoutBuffer.slice(
            actualClose + FILE_OP_CLOSE2.length + (closeIdx !== -1 ? 1 : 0)
          );
          inFileOpBlock = false;
          await this.executeFileOp(fileOpBuffer.trim());
          fileOpBuffer = "";
...
`);
    ...
    this.emit("fileOpStart", op);
    const result = await this.fileOpsHandler.execute(op);
    this.emit("fileOpResult", op, result);
  }
  dispose() {
    var _a3;
...
async function createRunner(agentId, settings, detectionResults, fileOpsHandler) {
  var _a3, _b, _c2, _d, _e2, _f, _g, _h, _i;
  const agentConfig = settings.agents[agentId];
  const detection = detectionResults.find((r2) => r2.id === agentId);
  const provider = PROVIDERS.find((p2) => p2.agentId === agentId);
  const accessMode = agentConfig.accessMode;
  if (accessMode === "cli") {
    if (!(detection == null ? void 0 : detection.isInstalled)) {
      const runner = createErrorRunner("CLI binary not found. Please re-scan or switch to API mode.");
      return runner;
    }
    const adapter = AGENT_ADAPTERS.find((a2) => a2.id === agentId);
    if (!adapter) {
      return createErrorRunner(`No CLI adapter found for agent '${agentId}'.`);
    }
    const extraArgs = agentConfig.extraArgs ? agentConfig.extraArgs.split(/\s+/).filter((s2) => s2.length > 0) : [];
    const yoloArgs = agentConfig.yoloMode ? (_a3 = adapter.yoloArgs) != null ? _a3 : [] : [];
  ...
  if (accessMode === "api") {
    ...
    if (agentId === "openai-compat") {
      const baseUrl = (_b = agentConfig.openaiCompatBaseUrl) == null ? void 0 : _b.trim();
      if (!baseUrl) {
        return createErrorRunner("No base URL configured. Set the URL in Settings \u2192 AI Agent Sidebar.");
      }
      const apiKey2 = (_c2 = agentConfig.openaiCompatApiKey) != null ? _c2 : "";
      const selectedModel2 = (_e2 = (_d = agentConfig.selectedModel) == null ? void 0 : _d.trim()) != null ? _e2 : provider.defaultModel;
      if (!selectedModel2) {
        return createErrorRunner("No model configured. Set the model name in Settings \u2192 AI Agent Sidebar.");
      }
      return new AgentApiRunner(agentId, apiKey2, selectedModel2, fileOpsHandler, settings.debugMode, baseUrl);
    }
    const settingsApiKey = (_f = agentConfig.apiKey) == null ? void 0 : _f.trim();
    if (!settingsApiKey && (!(detection == null ? void 0 : detection.hasApiKey) || !detection.apiKeyVar)) {
      return createErrorRunner("API key not detected. Set the required environment variable in your shell profile.");
    }
    const shellEnv = await resolveShellEnv();
    const apiKey = settingsApiKey != null ? settingsApiKey : shellEnv[detection.apiKeyVar];
    if (!apiKey) {
      return createErrorRunner("API key environment variable is set but empty.");
    }
    const selectedModel = (_g = agentConfig.selectedModel) != null ? _g : provider.defaultModel;
    const model = MODEL_FORMAT.test(selectedModel) ? selectedModel : provider.defaultModel;
    const settingsBaseUrl = (_h = agentConfig.apiBaseUrl) == null ? void 0 : _h.trim();
    const rawEnvBaseUrl = provider.apiBaseUrlEnvVar ? (_i = shellEnv[provider.apiBaseUrlEnvVar]) == null ? void 0 : _i.trim() : void 0;
    const rawBaseUrl = settingsBaseUrl || rawEnvBaseUrl;
    const baseURL = rawBaseUrl && isValidBaseUrl(rawBaseUrl) ? rawBaseUrl : void 0;
...
function createErrorRunner(message) {
  class ErrorRunner extends import_events3.EventEmitter {
    run(_messages, _context) {
...
// src/AgentSidebarView.ts
var AGENT_SIDEBAR_VIEW_TYPE = "agent-sidebar-view";
var AgentSidebarView = class extends import_obsidian3.ItemView {
  ...
  async buildTabs() {
    ...
    const detectionResults = (_a3 = this.plugin.agentDetector.getCache()) != null ? _a3 : [];
  ...
  getEnabledAgents(detectionResults) {
    return PROVIDERS.map((provider) => {
      var _a3, _b, _c2;
      const detection = detectionResults.find((r2) => r2.id === provider.agentId);
      if (!detection) return null;
      const config = this.plugin.settings.agents[detection.id];
      if (!config.enabled) return null;
      const hasSettingsApiKey = Boolean((_c2 = (_a3 = config.apiKey) == null ? void 0 : _a3.trim()) != null ? _c2 : (_b = config.openaiCompatApiKey) == null ? void 0 : _b.trim());
      if (!detection.isInstalled && !detection.hasApiKey && !hasSettingsApiKey) return null;
      return detection;
    }).filter((r2) => r2 !== null);
  }
  async addAgentTab(detection) {
    var _a3;
    ...
    const provider = PROVIDERS.find((p2) => p2.agentId === detection.id);
  ...
  hasConversationHistory(agentId) {
...
var modelListCache = {};
function createToggle(parent, checked, disabled, onChange, testId) {
...
var AgentSidebarSettingTab = class extends import_obsidian4.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.detectionResults = [];
    this.cardUpdaters = /* @__PURE__ */ new Map();
    this.plugin = plugin;
  }
  display() {
    ...
    this.detectionResults = cached != null ? cached : [];
  ...
  streamScan(rescanBtn) {
    const TIMEOUT_MS = 1e4;
    const timeoutId = setTimeout(() => {
      var _a3;
      for (const provider of PROVIDERS) {
        const agentId = provider.agentId;
        if (!this.detectionResults.find((r2) => r2.id === agentId)) {
          ...
          (_a3 = this.cardUpdaters.get(agentId)) == null ? void 0 : _a3(fallback);
        }
      }
      if (rescanBtn) {
    ...
    void this.plugin.agentDetector.detectStream(AGENT_ADAPTERS, (result) => {
      var _a3;
      this.detectionResults = [
        ...this.detectionResults.filter((r2) => r2.id !== result.id),
        result
      ];
      (_a3 = this.cardUpdaters.get(result.id)) == null ? void 0 : _a3(result);
    }).then(() => {
  ...
  renderProviders(root) {
    ...
    rescanBtn.addEventListener("click", () => {
      ...
      for (const key of PROVIDERS.map((p2) => p2.agentId)) delete modelListCache[key];
  ...
  renderProviderCard(parent, provider) {
    var _a3;
    const agentId = provider.agentId;
    const card = parent.createDiv({ cls: "ais-card" });
    card.dataset.testid = `ai-agent-settings-section-${provider.id}`;
    const renderContent = (detection) => {
      var _a4, _b, _c2;
      card.empty();
      card.className = "ais-card";
      const isChecking = detection === null;
      const isInstalled = (_a4 = detection == null ? void 0 : detection.isInstalled) != null ? _a4 : false;
      const hasApiKey = (_b = detection == null ? void 0 : detection.hasApiKey) != null ? _b : false;
      const config = this.plugin.settings.agents[agentId];
      const canEnable = isInstalled || hasApiKey || Boolean((_c2 = config.apiKey) == null ? void 0 : _c2.trim()) || provider.apiKeyOptional === true;
      if (!isChecking) {
    ...
    renderContent((_a3 = this.detectionResults.find((r2) => r2.id === agentId)) != null ? _a3 : null);
    this.cardUpdaters.set(agentId, renderContent);
  }
  renderCardHeader(card, provider, agentId, detection, isInstalled, hasApiKey, canEnable, isEnabled, isChecking) {
    var _a3, _b, _c2;
    ...
    if (provider.cliSupported) {
      ...
      if (!isChecking) {
        dot.title = isInstalled ? `Detected at ${(_a3 = detection == null ? void 0 : detection.path) != null ? _a3 : ""}` : `'${provider.cliCommand}' not found in PATH`;
      }
    }
    if (provider.apiSupported && provider.apiKeyEnvVar) {
      ...
      if (!isChecking) {
        if (hasApiKey) {
          const foundVar = (_b = detection == null ? void 0 : detection.apiKeyVar) != null ? _b : provider.apiKeyEnvVar;
          dot.title = `Detected: ${foundVar}. Validity is confirmed on first use.`;
        } else {
          const allVars = [provider.apiKeyEnvVar, ...(_c2 = provider.fallbackApiKeyEnvVars) != null ? _c2 : []].join(" or ");
    ...
    createToggle(
      ...
      (val) => {
        ...
        void this.plugin.saveSettings().then(() => {
          var _a4;
          void ((_a4 = this.plugin.getAgentSidebarView()) == null ? void 0 : _a4.refreshTabs());
        });
      },
      `ai-agent-enable-toggle-${agentId}`
    );
  }
  renderCardBody(card, provider, agentId, isInstalled, hasApiKey, currentMode, config) {
    var _a3;
    ...
    if (provider.cliSupported && provider.apiSupported) {
      ...
      checkbox.disabled = !isInstalled && !hasApiKey && !((_a3 = this.plugin.settings.agents[agentId].apiKey) == null ? void 0 : _a3.trim());
      ...
      checkbox.addEventListener("change", () => {
        var _a4, _b;
        const newMode = checkbox.checked ? "api" : "cli";
        const revert = () => {
        ...
        const performSwitch = () => {
          ...
          void this.plugin.saveSettings();
          void ((_a5 = this.plugin.getAgentSidebarView()) == null ? void 0 : _a5.refreshTabs());
          fields.empty();
          this.populateCardFields(fields, provider, agentId, newMode, config);
        };
        const hasHistory = (_b = (_a4 = this.plugin.getAgentSidebarView()) == null ? void 0 : _a4.hasConversationHistory(agentId)) != null ? _b : false;
        if (hasHistory) {
          revert();
          new ConfirmModal(
            this.app,
  ...
  populateCardFields(container, provider, agentId, mode, config) {
    var _a3, _b;
    if (mode === "cli" && provider.cliSupported) {
      const adapter = AGENT_ADAPTERS.find((a2) => a2.id === agentId);
      if ((_a3 = adapter == null ? void 0 : adapter.yoloArgs) == null ? void 0 : _a3.length) {
      ...
      const input = fieldRow.createEl("input", {
        cls: "ais-field-input",
        attr: { type: "text", placeholder: (_b = provider.cliArgsPlaceholder) != null ? _b : "e.g. --flag value", "data-testid": `ai-agent-extra-args-${agentId}` }
      });
      input.value = config.extraArgs;
      input.addEventListener("change", () => {
        if (SHELL_INJECTION_PATTERN.test(input.value)) {
          new import_obsidian4.Notice("Extra args contain unsafe characters. Remove ; | & ` $( > and try again.");
          input.value = config.extraArgs;
          return;
        }
  ...
  renderOpenAICompatFields(container, agentId) {
    var _a3, _b, _c2;
    const config = this.plugin.settings.agents[agentId];
    ...
    urlInput.value = (_a3 = config.openaiCompatBaseUrl) != null ? _a3 : "";
    ...
    keyInput.value = (_b = config.openaiCompatApiKey) != null ? _b : "";
    ...
    modelInput.value = (_c2 = config.selectedModel) != null ? _c2 : "";
    modelInput.addEventListener("change", () => {
      config.selectedModel = modelInput.value.trim() || void 0;
      void this.plugin.saveSettings();
    });
  }
  renderModelField(body, agentId, defaultModel) {
    var _a3;
    const config = this.plugin.settings.agents[agentId];
    const provider = PROVIDERS.find((p2) => p2.agentId === agentId);
    ...
    if (cached) {
      this.buildModelSelect(fieldRow, agentId, cached, (_a3 = config.selectedModel) != null ? _a3 : provider.defaultModel, defaultModel);
      return;
    }
    ...
    void (async () => {
      ...
      let models;
      try {
        const timeout = new Promise((_2, rej) => setTimeout(() => rej(new Error("timeout")), 1e4));
        ...
        this.buildModelSelect(fieldRow, agentId, models, (_a4 = config.selectedModel) != null ? _a4 : provider.defaultModel, defaultModel);
        return;
      }
      loading.remove();
      this.buildModelSelect(fieldRow, agentId, models, (_b = config.selectedModel) != null ? _b : provider.defaultModel, defaultModel);
    })();
  }
  buildModelSelect(parent, agentId, models, currentModel, defaultModel) {
    const config = this.plugin.settings.agents[agentId];
    ...
    for (const m2 of models) {
      const opt = sel.createEl("option", { text: m2, attr: { value: m2 } });
      if (m2 === effective) opt.selected = true;
    }
  ...
  async fetchModels(agentId) {
    const detection = this.detectionResults.find((r2) => r2.id === agentId);
    if (!(detection == null ? void 0 : detection.hasApiKey) || !detection.apiKeyVar) return this.getDefaultModels(agentId);
    const { resolveShellEnv: resolveShellEnv2 } = await Promise.resolve().then(() => (init_shellEnv(), shellEnv_exports));
    const env = await resolveShellEnv2();
    const apiKey = env[detection.apiKeyVar];
    if (!apiKey) return this.getDefaultModels(agentId);
    if (agentId === "claude") {
      const r2 = await (0, import_obsidian4.requestUrl)({
        url: "https://api.anthropic.com/v1/models",
        headers: { "x-api-key": apiKey, "anthropic-version": "2023-06-01" }
      });
      if (r2.status !== 200) return this.getDefaultModels(agentId);
      const d2 = r2.json;
      return d2.data.map((m2) => m2.id);
    }
    if (agentId === "codex") {
      const r2 = await (0, import_obsidian4.requestUrl)({
      ...
      if (r2.status !== 200) return this.getDefaultModels(agentId);
      const d2 = r2.json;
      return d2.data.map((m2) => m2.id).filter((id) => id.startsWith("gpt-") || /^o\d/.test(id)).sort();
    }
    if (agentId === "gemini") {
      const r2 = await (0, import_obsidian4.requestUrl)({
  ...
  getDefaultModels(agentId) {
    ...
    return (_a3 = map[agentId]) != null ? _a3 : [];
  }
  // ── Global options ────────────────────────────────────────────────────────────
  renderGlobal(root) {
    ...
    this.renderGlobalRow(
      ...
      (v2) => {
        this.plugin.settings.debugMode = v2;
        void this.plugin.saveSettings();
      }
    );
  }
  renderGlobalRow(parent, name, desc, checked, onChange) {
  ...
  renderFooter(root) {
    ...
    const footerItems = [
      {
        title: "Documentation",
        desc: "README, setup guide, and usage",
        url: `${REPO}#readme`,
      ...
      {
        title: "Discussions",
        desc: "Questions, ideas, and community",
      ...
      },
      {
        ...
        desc: "Source code, releases, and wiki",
        url: REPO,
    ...
    const grid = root.createDiv({ cls: "ais-footer-links" });
    for (const item of footerItems) {
      const link = grid.createEl("a", { cls: "ais-footer-link" });
      ...
      const iconEl = link.createDiv({ cls: "ais-footer-link-icon" });
      const doc = new DOMParser().parseFromString(item.icon, "image/svg+xml");
      iconEl.appendChild(document.adoptNode(doc.documentElement));
      const meta = link.createDiv({ cls: "ais-footer-link-meta" });
      meta.createDiv({ cls: "ais-footer-link-title", text: item.title });
      meta.createDiv({ cls: "ais-footer-link-desc", text: item.desc });
    }
  }
};
...
// src/main.ts
var AgentSidebarPlugin = class extends import_obsidian5.Plugin {
  constructor() {
    super(...arguments);
  ...
  getAgentSidebarView() {
    ...
    return (leaf == null ? void 0 : leaf.view) instanceof AgentSidebarView ? leaf.view : null;
  }
  onunload() {
  }
  async loadSettings() {
    ...
    const agentIds = ["claude", "codex", "gemini", "copilot", "openai-compat"];
    ...
    for (const id of agentIds) {
      if (!this.settings.agents[id]) {
        this.settings.agents[id] = { enabled: false, extraArgs: "", yoloMode: false, accessMode: (_a3 = defaultAccessModes[id]) != null ? _a3 : "cli" };
      } else {
        ...
        if (this.settings.agents[id].yoloMode === void 0) {
  ...
  async activateSidebar() {
    ...
    if (!leaf) {
      leaf = (_a3 = workspace.getRightLeaf(false)) != null ? _a3 : workspace.getLeaf(true);
